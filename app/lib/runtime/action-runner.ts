...
export class ActionRunner {
  ...
  async #runStartAction(action: ActionState) {
    ...
    // Sanitize unsupported commands for WebContainer (e.g., pkill/killall/lsof/fuser)
    const sanitizedContent = action.content
      // No-op unsupported mass-kill commands while preserving command chaining
      .replace(/\bpkill\b[^;&\n|]*/g, 'true')
      .replace(/\bkillall\b[^;&\n|]*/g, 'true')
      // Replace common kill-port patterns that rely on lsof/fuser
      .replace(/kill\s+-9\s+\$\([^)]*\)/g, 'true')
      .replace(/\bfuser\b[^;&\n|]*/g, 'true');

    const resp = await shell.executeCommand(this.runnerId.get(), sanitizedContent, () => {
      logger.debug(`[${action.type}]:Aborting Action\n\n`, action);
      action.abort();
    });
    logger.debug(`${action.type} Shell Response: [exit code:${resp?.exitCode}]`);
  ...
  async #runFileAction(action: ActionState) {
    ...
    try {
      ...
      // Add a small delay to ensure filesystem sync, especially for critical files like package.json
      if (relativePath.endsWith('package.json') || relativePath.endsWith('vite.config.js') || relativePath.endsWith('vite.config.ts')) {
  ...
  async #validateShellCommand(command: string): Promise<{
    ...
    // Make npm/yarn/pnpm commands non-interactive
    if (trimmedCommand.match(/^(npm|yarn|pnpm)\s+(install|i|add)/)) {
      const modifiedCommand = trimmedCommand
        .replace(/^npm\s+install(?!\s+(-|--))/, 'npm install --yes --no-audit --no-fund')
        .replace(/^npm\s+i(?!\s+(-|--))/, 'npm install --yes --no-audit --no-fund')
        .replace(/^yarn\s+add(?!\s+(-|--))/, 'yarn add --non-interactive')
        .replace(/^pnpm\s+(install|add|i)(?!\s+(-|--))/, 'pnpm $1 --reporter=silent');
      
      if (modifiedCommand !== trimmedCommand) {
        return {
          shouldModify: true,
          modifiedCommand,
          warning: 'Made package manager command non-interactive',
        };
      }
    }
