...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- Brainiac: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via Brainiac bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/brainiac';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'brainiac-bug-reporter',
    });
