...
async function fetchRepoContentsZip(repo: string, githubToken?: string) {
  ...
  if (!releaseResponse.ok) {
    // If no releases, fall back to the main branch
    console.log(`No releases found for ${repo}, falling back to main branch`);
  ...
  if (!zipResponse.ok) {
    // If zipball fails, fall back to the main branch
    console.log(`Failed to fetch release zipball for ${repo}, falling back to main branch`);
    return await fetchRepoContentsCloudflare(repo, githubToken);
  }
...
export async function loader({ request, context }: { request: Request; context: any }) {
  ...
  try {
    ...
    try {
      ...
      console.log(`Primary method failed for ${repo}, trying fallback:`, primaryError);
    ...
    // Return empty template instead of error to allow the app to continue
    return json([]);
  }
}
